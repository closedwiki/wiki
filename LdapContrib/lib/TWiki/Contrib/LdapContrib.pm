...
sub new {
  ...
  my $this = {
    ...
    userScope=>$TWiki::cfg{Ldap}{UserScope}
      || 'sub',
    ...
    isConnected=>0,
    maxCacheAge=>$TWiki::cfg{Ldap}{MaxCacheAge},
    preCache=>$TWiki::cfg{Ldap}{Precache},
...
sub cacheUserFromEntry {
  ...
  foreach my $groupName (keys %groupNames) {
    if (defined $data->{"GROUP2UNCACHEDMEMBERSDN::$groupName"}) {
      my $dnList = 
        TWiki::Sandbox::untaintUnchecked($data->{"GROUP2UNCACHEDMEMBERSDN::$groupName"}) || '';
...
sub getWikiNameOfLogin {
  ...
  return TWiki::Sandbox::untaintUnchecked($data->{"U2W::$loginName"});
}
...
sub getLoginOfWikiName {
  ...
  unless ($loginName) {
    my $alias = $this->{wikiNameAliases}{$wikiName};
    $loginName = TWiki::Sandbox::untaintUnchecked($data->{"W2U::$alias"})
      if defined($alias);
  }
...
sub getAllWikiNames {
  ...
  my $wikiNames = TWiki::Sandbox::untaintUnchecked($data->{WIKINAMES}) || '';
...
sub getAllLoginNames {
  ...
  my $loginNames = TWiki::Sandbox::untaintUnchecked($data->{LOGINNAMES}) || '';
  my @loginNames = split(/\s*,\s*/,$loginNames);
  return \@loginNames;
}
...
sub getAllUnknownUsers {
  ...
  my $wikiNames = TWiki::Sandbox::untaintUnchecked($data->{UNKWNUSERS}) || '';
  my @wikiNames = split(/\s*,\s*/,$wikiNames);
  return \@wikiNames;
}
...
sub getAllUnknownGroups {
  ...
  my $wikiNames = TWiki::Sandbox::untaintUnchecked($data->{UNKWNGROUPS}) || '';
  my @wikiNames = split(/\s*,\s*/,$wikiNames);
  return \@wikiNames;
}
