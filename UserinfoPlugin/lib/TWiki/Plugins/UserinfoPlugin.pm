...
sub initPlugin {
    ...
    # register the _USERINFO function to handle %USERINFO{...}%
    TWiki::Func::registerTagHandler( 'USERINFO', \&_USERINFO );
    TWiki::Func::registerTagHandler( 'WIKINAME', \&_WIKINAME );
    TWiki::Func::registerTagHandler( 'USERNAME', \&_USERNAME );
    TWiki::Func::registerTagHandler( 'WIKIUSERNAME', \&_WIKIUSERNAME );

    # Plugin correctly initialized
    return 1;
}

# The function used to handle the %USERINFO{...}% variable
...
sub _WIKINAME {
    my($session, $params, $theTopic, $theWeb) = @_;
    $params->{format} = $session->{prefs}->getPreferencesValue( 'WIKINAME' ) ||
            '%USERINFO{format="$wikiname"}%';

    return _USERINFO($session, $params);
}

sub _USERNAME {
    my($session, $params, $theTopic, $theWeb) = @_;
            '%USERINFO{format="$wikiname"}%';
    $params->{format} = $session->{prefs}->getPreferencesValue( 'USERNAME' ) ||
...
sub _WIKIUSERNAME {
    my($session, $params, $theTopic, $theWeb) = @_;
            '%USERINFO{format="$wikiname"}%';
    $params->{format} = $session->{prefs}->getPreferencesValue( 'WIKIUSERNAME' ) ||
