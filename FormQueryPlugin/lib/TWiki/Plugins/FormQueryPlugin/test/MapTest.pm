...
sub test_parse1 {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map( "a = one bit=\"two\" c" );
...
sub test_parse2 {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map( "aname = one,b = \"two\",c" );
  $this->assert_not_null($attrs);
...
sub test_parse3 {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map( "x.y=one" );
  $this->assert_not_null($attrs);
...
sub test_parse4 {
  my $this = shift;
  my $attrs;
  eval { new FormQueryPlugin::Map( "topic=MacroReqDetails area = \"Signal Integrity\" status=\"Assigned\" release=\"2003.06|All product=\"Fsim\"" ); };
...
sub test_multipleDefs1 {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map( "a = one a=\"two\"" );
  $this->assert_not_null($attrs);
...
sub testMultipleDefs2 {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map( "a=\"two\" a" );
  $this->assert_not_null($attrs);
...
sub testMultipleDefs4 {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map( "a a = one" );
  $this->assert_not_null($attrs);
  $this->assert_str_equals("one", $attrs->remove("a"));
}
...
sub testStringOnOwn {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map( "\"able cain\" a=\"no\"" );
  $this->assert_not_null($attrs);
...
sub test_big {
  ...
  my $attrs = new FormQueryPlugin::Map( $str );
}
...
sub test_kandv {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map( "a=A b=B c=C d=D" );
...
sub test_search {
  my $this = shift;
  my $attrs = new FormQueryPlugin::Map();
  $attrs->set("a", new FormQueryPlugin::Map("f=A"));
  $attrs->set("b", new FormQueryPlugin::Map("f=B"));
  $attrs->set("c", new FormQueryPlugin::Map("f=C"));
  $attrs->set("d", new FormQueryPlugin::Map("f=D"));
  $this->assert_equals(4, $attrs->size());
  my $search = new FormQueryPlugin::Search("f=~'(B|C)'");
