...
sub set_up {
  $submap = new FormQueryPlugin::Map("a=1 b=2");
  $array = new FormQueryPlugin::Array();
  $map = new FormQueryPlugin::Map();
...
sub test_writeRead {
  ...
  my $a = new FormQueryPlugin::Archive("data.dat", "w");
  $a->writeInt(1949);
  $a->writeObject($map);
  $a->close();

  $a = new FormQueryPlugin::Archive("data.dat", "r");
...
sub test_writeDuringRead {
  my $this = shift;
  my $a1 = new FormQueryPlugin::Archive("data.dat", "w");
  $a1->writeByte('Y');
  $a1->close();

  $a1 = new FormQueryPlugin::Archive("data.dat", "r");
  ...
  my $pid = open SUB, "| perl -e 'use lib \"../..\";use FormQueryPlugin::Archive;\$a = new FormQueryPlugin::Archive(\"data.dat\", \"w\");\$a->writeByte(\"X\");\$a->close();1;'";
  sleep(1);
