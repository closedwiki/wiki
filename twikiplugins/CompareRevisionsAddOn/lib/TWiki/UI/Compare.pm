...
my $class_add = 'twikiCompareAdd';
my $class_del = 'twikiCompareDelete';
my $class_c1  = 'twikiCompareChange1';
my $class_c2  = 'twikiCompareChange2';
my $interweave;
my $context;
my $scripturl;
...
sub compare {
    ...
    my ($tmpl_before, $tmpl_us, $tmpl_u, $tmpl_c, $tmpl_a, $tmpl_d, $tmpl_after);
	...
	if ($action->[0] eq 'c' &&
		...
		($text1, $text2) = _findSubChanges($action->[1], $action->[2]);
	    }
	}

	# Otherwise format this particular action

	($text1, $text2) = _getTextFromAction($action) unless $text1 || $text2;

	my $tmpl = $action->[0] eq 'u' ? $tmpl_u :
...
#    my $revisions = "";
	...
	if( $i == $rev2 && $i-1 == $rev1 ) {
	  ...
	  $revisions .= "  <a href=\"%SCRIPTURLPATH%/compare%SCRIPTSUFFIX%/%WEB%/%TOPIC%?rev1=$i&rev2=".($i-1).($query->param('skin')?'&skin='.$query->param('skin'):'').($query->param('context')?'&context='.$query->param('context'):'').'&render='.$renderStyle.'">&lt;</a>';
	}
	$i--; 
    }
    ...
    $tmpl_after = TWiki::Func::expandCommonVariables($tmpl_after, $topic, $webName);
...
sub _getTree {
    ...
    $_->delete foreach ( $tree->look_down( '_tag' => 'p',
				           sub{ $_[0]->is_empty } ) );
...
sub _findSubChanges {
	...
	return $e1 eq $e2 ? ($e1, $interweave ? '' : $e2) 
			  : _compareText($e1, $e2);
...
sub _compareText {
	...
	if ($action->[0] eq '+') {
	    ($interweave ? $ctext1 : $ctext2) .=
		     '<span class="'.$class_add.'">'.
		     $action->[2].
		     '</span> ';
	} elsif ($action->[0] eq '-') {
	    $ctext1.='<span class="'.$class_del.'">'.
		     $action->[1].
		     '</span> ';
	} elsif ($action->[0] eq 'c') {
	    $ctext1.='<span class="'.$class_c1.'">'.
		     $action->[1].
		     '</span> ';
	    ($interweave ? $ctext1 : $ctext2) .=
	             '<span class="'.$class_c2.'">'.
