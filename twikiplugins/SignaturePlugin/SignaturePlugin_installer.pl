# Install script for SignaturePlugin
#
...
DONE
    ...
    unless ( $ok ) {
        print <<DONE;
*** SignaturePlugin depends on $dep->{type} package $dep->{name} $dep->{version}
...
sub uninstall {
    ...
    unless ( $#dead > 1 ) {
        print STDERR 'No part of SignaturePlugin is installed';
        return 0;
    }
    print 'To uninstall SignaturePlugin, the following files will be deleted:'.$NL;
    print join( ', ', @dead );
    my $reply = ask('Are you SURE you want to uninstall SignaturePlugin?');
    if( $reply ) {
        # >>>> PREUNINSTALL
    ...
    print '### SignaturePlugin uninstalled ###'.$NL;
    return 1;
}
...
DONE
    ...
    print $NL.'### SignaturePlugin installed';
...
sub usage {
    print STDERR <<'DONE';
Usage:SignaturePlugin_installer [-a] install
      SignaturePlugin_installer [-a] uninstall
      SignaturePlugin_installer [-a] upgrade
...
SignaturePlugin even if they have been locally modified.
...
print $NL,'### SignaturePlugin Installer ###',$NL,$NL;
my $n = 0;
my $action = 'install';
while ( $n < scalar( @ARGV ) ) {
    if( $ARGV[$n] eq '-a' ) {
...
if( $action eq 'upgrade' ) {
    ...
    print <<DONE;
I would like to uninstall SignaturePlugin before upgrading, to
...
DONE
    ...
    installPackage( "$PACKAGES_URL/SignaturePlugin/", 'SignaturePlugin' );
}
