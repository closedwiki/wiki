# Install script for MultiEditPlugin
#
...
my $twiki;
my $NL = "\n";
my %manifest = ( 'bin/editonesection'=>0550,'data/TWiki/MultiEditPlugin.txt'=>0660,'lib/TWiki/Plugins/MultiEditPlugin.pm'=>0440,'pub/TWiki/MultiEditPlugin/multieditplugin_example.jpg'=>0440,'lib/TWiki/Plugins/MultiEditPlugin/build.pl'=>0550, );
my @deps = ( { name=>'TWiki::Contrib::EditContrib', type=>'perl',version=>'>=1.001',description=>'Required. Download and install from the TWiki:Plugins/EditContrib code library.', trigger=>1 } );
my $dakar;
my %available;
my $lwp;
...
DONE
    ...
    unless ( $ok ) {
        print <<DONE;
*** MultiEditPlugin depends on $dep->{type} package $dep->{name} $dep->{version}
...
sub uninstall {
    ...
    unless ( $#dead > 1 ) {
        print STDERR 'No part of MultiEditPlugin is installed';
        return 0;
    }
    print 'To uninstall MultiEditPlugin, the following files will be deleted:'.$NL;
    print join( ', ', @dead );
    my $reply = ask('Are you SURE you want to uninstall MultiEditPlugin?');
    if( $reply ) {
        # >>>> PREUNINSTALL
    ...
    print '### MultiEditPlugin uninstalled ###'.$NL;
    return 1;
}
...
DONE
    ...
    print $NL.'### MultiEditPlugin installed';
...
sub usage {
    print STDERR <<'DONE';
Usage:MultiEditPlugin_installer [-a] install
      MultiEditPlugin_installer [-a] uninstall
      MultiEditPlugin_installer [-a] upgrade
...
MultiEditPlugin even if they have been locally modified.
...
print $NL,'### MultiEditPlugin Installer ###',$NL,$NL;
my $n = 0;
my $action = 'install';
while ( $n < scalar( @ARGV ) ) {
    if( $ARGV[$n] eq '-a' ) {
...
if( $action eq 'upgrade' ) {
    ...
    print <<DONE;
I would like to uninstall MultiEditPlugin before upgrading, to
...
DONE
    ...
    installPackage( "$PACKAGES_URL/MultiEditPlugin/", 'MultiEditPlugin' );
}
