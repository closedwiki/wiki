# Install script for ExcelImportExportPlugin
#
...
my $twiki;
my $NL = "\n";
my %manifest = ( 'lib/TWiki/Plugins/ExcelImportExportPlugin.pm'=>0440,'lib/TWiki/Plugins/ExcelImportExportPlugin/Import.pm'=>0440,'lib/TWiki/Plugins/ExcelImportExportPlugin/Export.pm'=>0440,'data/TWiki/ExcelImportExportPlugin.txt'=>0660,'bin/excel2topics'=>0550,'bin/topics2excel'=>0550,'bin/table2excel'=>0550,'data/Sandbox/ExcelExport1.txt'=>0660,'data/Sandbox/ExcelImport1.txt'=>0660,'data/Sandbox/IssueExcelExportImport.txt'=>0660,'data/Sandbox/IssueTrackingForm.txt'=>0660,'pub/Sandbox/IssueExcelExportImport/IssueExcelExportImport.xls'=>0660, );
my @deps = ( { name=>'Spreadsheet::ParseExcel', type=>'cpan',version=>'>=0',description=>'Required. Available from [[http://search.cpan.org][CPAN]].', trigger=>1 }
,{ name=>'Spreadsheet::WriteExcel', type=>'cpan',version=>'>=0',description=>'Required. Available from [[http://search.cpan.org][CPAN]].', trigger=>1 }
,{ name=>'Date::Manip', type=>'cpan',version=>'>=0',description=>'Required. Available from [[http://search.cpan.org][CPAN]].', trigger=>1 } );
my $dakar;
my %available;
my $lwp;
...
DONE
    ...
    unless ( $ok ) {
        print <<DONE;
*** ExcelImportExportPlugin depends on $dep->{type} package $dep->{name} $dep->{version}
...
sub uninstall {
    ...
    unless ( $#dead > 1 ) {
        print STDERR 'No part of ExcelImportExportPlugin is installed';
        return 0;
    }
    print 'To uninstall ExcelImportExportPlugin, the following files will be deleted:'.$NL;
    print join( ', ', @dead );
    my $reply = ask('Are you SURE you want to uninstall ExcelImportExportPlugin?');
    if( $reply ) {
        # >>>> PREUNINSTALL
    ...
    print '### ExcelImportExportPlugin uninstalled ###'.$NL;
    return 1;
}
...
DONE
    ...
    print $NL.'### ExcelImportExportPlugin installed';
...
sub usage {
    print STDERR <<'DONE';
Usage:ExcelImportExportPlugin_installer [-a] install
      ExcelImportExportPlugin_installer [-a] uninstall
      ExcelImportExportPlugin_installer [-a] upgrade
...
ExcelImportExportPlugin even if they have been locally modified.
...
print $NL,'### ExcelImportExportPlugin Installer ###',$NL,$NL;
my $n = 0;
my $action = 'install';
while ( $n < scalar( @ARGV ) ) {
    if( $ARGV[$n] eq '-a' ) {
...
if( $action eq 'upgrade' ) {
    ...
    print <<DONE;
I would like to uninstall ExcelImportExportPlugin before upgrading, to
...
DONE
    ...
    installPackage( "$PACKAGES_URL/ExcelImportExportPlugin/", 'ExcelImportExportPlugin' );
}
