...
use strict;
use vars qw($isInitialized $debug);
use URI::Escape;
...
sub natSearch {
  ...
  if ($theSearchString =~ /^[A-Z]/) { 
    ...
    if (&TWiki::Func::topicExists($web, $theSearchString)) {
      my $viewUrl = &TWiki::Func::getViewUrl($web, $theSearchString);
