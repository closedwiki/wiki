# Install script for CalendarPlugin
#
...
sub satisfy {
	 ...
	 unless ( $ok ) {
		  print "*** CalendarPlugin depends on package $dep->{name} $dep->{version},\n";
...
sub usage {
	 print STDERR "Usage:\tCalendarPlugin_installer [-a] install\n";
	 print STDERR "\tCalendarPlugin_installer uninstall\n";
	 print STDERR "Install or uninstall CalendarPlugin. Default is to install. Should be run\n";
...
my @manifest = ( 	"./data/TWiki/CalendarPlugin.txt", # 
	"./data/TWiki/CalendarPlugin.txt,v", # 
	"./lib/TWiki/Plugins/CalendarPlugin.pm", # 
	"./lib/TWiki/Plugins/CalendarPlugin/build.pl", # 
	"./pub/TWiki/CalendarPlugin/exclam.gif", # 
	"./CalendarPlugin_installer.pl", # 
	"CalendarPlugin_installer.pl", # Install script
 );
...
if( $install ) {
	 ...
	 my $unsatisfied = 0;
	 foreach my $dep ( ( { name=>"Date::Calc", type=>"perl",version=>">=1.000",description=>"Required. Download from CPAN and install.", trigger=>1 }
,{ name=>"HTML::CalendarMonthSimple", type=>"perl",version=>">=1.23",description=>"Required. Download from CPAN and install.", trigger=>1 } ) ) {
		  unless ( satisfy( $dep ) ) {
				$unsatisfied++;
		  }
	 }
	 ...
	 unless ( $#dead > 1 ) {
		  die "No part of CalendarPlugin is installed";
	 }
	 print "To uninstall CalendarPlugin, the following files will be deleted:\n";
	 print join( ", ", @dead );
	 my $reply;
	 print "\nAre you SURE you want to uninstall CalendarPlugin? [y/n] ";
	 while ( ( $reply = <STDIN> ) !~ /^[yn]/i ) {
		  print "Please answer yes or no\n";
	 }
	 if( $reply =~ /^y/i ) {
	 ...
	 print "### CalendarPlugin uninstalled ###\n";
}
