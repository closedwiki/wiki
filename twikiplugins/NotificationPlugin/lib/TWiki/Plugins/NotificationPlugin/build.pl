...
{ package NotificationPluginBuild;

  @NotificationPluginBuild::ISA = ( "TWiki::Contrib::Build" );

  sub new {
    my $class = shift;
    return bless( $class->SUPER::new( "NotificationPlugin", "NotificationPlugin" ), $class );
  }
...
$build = new NotificationPluginBuild();
