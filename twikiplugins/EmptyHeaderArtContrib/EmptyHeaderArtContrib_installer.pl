# Install script for EmptyHeaderArtContrib
#
...
my $twiki;
my $NL = "\n";
my %manifest = ( 'data/TWiki/EmptyHeaderArtContrib.txt'=>508,'lib/TWiki/Contrib/EmptyHeaderArtContrib.pm'=>508,'data/Sandbox/PluginTestEmptyHeaderArtContrib.txt'=>508, );
my @deps = (  );
my $dakar;
my %available;
my $lwp;
...
DONE
	 ...
	 unless ( $ok ) {
		  print <<DONE;
*** EmptyHeaderArtContrib depends on $dep->{type} package $dep->{name} $dep->{version}
...
sub uninstall {
	 ...
	 unless ( $#dead > 1 ) {
		  print STDERR 'No part of EmptyHeaderArtContrib is installed';
		  return 0;
	 }
	 print 'To uninstall EmptyHeaderArtContrib, the following files will be deleted:'.$NL;
	 print join( ', ', @dead );
	 my $reply = ask('Are you SURE you want to uninstall EmptyHeaderArtContrib?');
	 if( $reply ) {
		  # >>>> PREUNINSTALL
	 ...
	 print '### EmptyHeaderArtContrib uninstalled ###'.$NL;
	 return 1;
}
...
DONE
	 ...
	 print $NL.'### EmptyHeaderArtContrib installed';
...
sub usage {
	 print STDERR <<'DONE';
Usage:EmptyHeaderArtContrib_installer [-a] install
		EmptyHeaderArtContrib_installer [-a] uninstall
		EmptyHeaderArtContrib_installer [-a] upgrade
...
EmptyHeaderArtContrib even if they have been locally modified.
...
print $NL,'### EmptyHeaderArtContrib Installer ###',$NL,$NL;
my $n = 0;
my $action = 'install';
while ( $n < scalar( @ARGV ) ) {
	 if( $ARGV[$n] eq '-a' ) {
...
if( $action eq 'upgrade' ) {
	 my $zip = 'EmptyHeaderArtContrib.zip';

	 return 0 unless getArchive( 'EmptyHeaderArtContrib' );

	 print <<DONE;
I would like to uninstall EmptyHeaderArtContrib before upgrading, to
...
DONE
	 ...
	 if( unzip( $zip )) {
		  ...
		  print `perl EmptyHeaderArtContrib_installer.pl install`;
	 }
}
