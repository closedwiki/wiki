...
sub warning  {TWiki::Func::writeWarning(@_); debug("WARNING" . $_[0], @_[1 .. $#_]); }
...
sub initPlugin
{
    ...
    $debug = TWiki::Func::getPreferencesFlag( $prefPrefix."DEBUG" );
...
sub afterSaveHandler
{
	...
	my %methodUsers = map { $userMethods{$user}{$method} ? ($_, \$users{$_}) : () } keys %users;
	my @userList = keys %methodUsers; # save current key list, so we can modify the hash
