...
sub mostraFluxograma {
    ...
    $style =
         TWiki::Func::extractNameValuePair( $params, 'tag-style' )
...
sub desenhaFluxograma {
    ...
    $caixa{'w'} =
         TWiki::Func::extractNameValuePair( $params, 'item-w' )
      || TWiki::Func::getPluginPreferencesValue('ITEM_WIDTH')
      || 140;
    $caixa{'h'} =
         TWiki::Func::extractNameValuePair( $params, 'item-h' )
      || TWiki::Func::getPluginPreferencesValue('ITEM_HEIGHT')
      || 40;
    $caixa{'areaX'} =
         TWiki::Func::extractNameValuePair( $params, 'area-w' )
      || TWiki::Func::getPluginPreferencesValue('ITEM_AREA_W')
      || 180;
    $caixa{'areaY'} =
         TWiki::Func::extractNameValuePair( $params, 'area-h' )
      || TWiki::Func::getPluginPreferencesValue('ITEM_AREA_H')
      || 70;
    $percentReduce =
         TWiki::Func::extractNameValuePair( $params, 'percent' )
      || TWiki::Func::getPluginPreferencesValue('PERCENT_IMG')
      || 70;
    $textSize =
         TWiki::Func::extractNameValuePair( $params, 'text-size' )
    ...
    my $status = system(
        $TWiki::cfg{Plugins}{$pluginName}{ImageMagickCmd}, # /usr/bin/convert
        "$myPub/flowchart_$topic.svg",
        '-resize', $percentReduce.'%x'.$percentReduce.'%',
        "$myPub/flowchart_$topic.png" );

    if( $status ) {
        TWiki::Func::writeWarning(
          "FlowchartPlugin: error while executing 'convert' command: $?"
        );
    }
}
...
'<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  <<      Get it from http://twiki.org/cgi-bin/view/Plugins/FlowchartPlugin        >>
