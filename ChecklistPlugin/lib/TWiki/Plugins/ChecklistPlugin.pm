...
### my ( $text, $topic, $web ) = @_;   # do not uncomment, use $_[0], $_[1]... instead
    ...
    eval {
	    ...
	    $_[0] =~ s/%CLI{(.*?)}%/&handleChecklistItem($1)/sge;
    };
...
sub doChecklistItemStateReset {
	my ($n, $state) = @_;
	TWiki::Func::writeDebug("- ${pluginName}::doChecklistItemStateReset($n,$state)") if $debug;
	if (!defined $state) {
		my @states=split /\|/, $options{'states'};
		$state=$states[0];
	}
	...
	&saveChecklistItemStateTopic($n) if (!$saveDone) && (($saveDone=!$saveDone));
}
# =========================
sub doChecklistItemStateChange {
	my ($id, $n, $lastState) = @_;
	TWiki::Func::writeDebug("- ${pluginName}::doChecklistItemStateChange($id,$n,$lastState)") if $debug;
