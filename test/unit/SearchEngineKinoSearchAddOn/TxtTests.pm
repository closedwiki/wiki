...
HERE
    $this->{twiki}->{store}->saveAttachment($this->{users_web}, "TopicWithTxtAttachment", "Simple_example.txt",
                                            $this->{twiki}->{user}, {file => "attachement_examples/Simple_example.txt"})
}
...
sub test_stringForFile {
    ...
    my $text2 = TWiki::Contrib::SearchEngineKinoSearchAddOn::Stringifier->stringFor('attachement_examples/Simple_example.txt');
