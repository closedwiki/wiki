...
sub actionNotify {
    ...
    if ($expr =~ /\bweb="([^" ,*]+)"/) {
        TWiki::Func::pushTopicContext($1, 'WebHome');
    }
...
sub doNotificationsCore {
    ...
    my $format =
      new TWiki::Plugins::ActionTrackerPlugin::Format( $hdr, $bdy, $orient,
        $textform, $changes );
...
sub _loadWebNotify {
    ...
    foreach my $line ( split( /\r?\n/, $text ) ) {
        if ( $line =~ /^\s+\*\s([\w\.]+)\s+-\s+([\w\-\.\+]+\@[\w\-\.\+]+)/o ) {
            ...
            my $addr = $2;
            $who =
              TWiki::Plugins::ActionTrackerPlugin::Action::_canonicalName(
                $who);
