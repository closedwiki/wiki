...
'Version mismatch between ActionTrackerPlugin and Plugins.pm $TWiki::Plugins::VERSION. 1.026 required.'
        );
    }
    ...
    TWiki::Func::registerTagHandler( 'ACTIONSEARCH', \&_handleActionSearch,
        'context-free' );
...
sub commonTagsHandler {
    ...
    my $as =
      TWiki::Plugins::ActionTrackerPlugin::ActionSet::load( $web, $topic,
        $otext, 1 );
    my $actionGroup;
    my $text = '';
...
sub _beforeNormalEdit {
    ...
    if ( $cc < $oc ) {
        ...
        my $as =
          TWiki::Plugins::ActionTrackerPlugin::ActionSet::load( $_[2], $_[1],
            $_[0], 1 );
        $_[0] = $as->stringify();
    }
}
...
sub _beforeActionEdit {
    ...
    my $tmpl =
      TWiki::Func::readTemplate( 'actionform', TWiki::Func::getSkin() );

    # Here we want to show the current time in same time format as the user
    # sees elsewhere in his browser on TWiki.
    my $date =
      TWiki::Func::formatTime( time(), undef,
...
sub afterEditHandler {
    ...
    my $action =
      TWiki::Plugins::ActionTrackerPlugin::Action::createFromQuery( $_[2],
        $_[1], $an, $query );
...
sub _handleActionSearch {
    ...
    if ( defined($fmts) || defined($hdrs) || defined($orient) ) {
        ...
        $fmt = new TWiki::Plugins::ActionTrackerPlugin::Format( $hdrs, $fmts,
            $orient, '', '' );
    }
    else {
        $fmt = $defaultFormat;
...
sub lazyInit {
    ...
    $options =
      TWiki::Plugins::ActionTrackerPlugin::Options::load( $web, $topic );
    ...
    if ( $options->{EXTRAS} ) {
        ...
        if ( defined($e) ) {
            TWiki::Func::writeWarning(
                "- TWiki::Plugins::ActionTrackerPlugin ERROR $e");
        }
...
sub _updateRESTHandler {
    ...
    try {
        ...
        my $web;
        ( $web, $topic ) =
          TWiki::Func::normalizeWebTopicName( undef, $topic );
        lazyInit( $web, $topic );
