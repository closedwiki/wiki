...
---+ package TWiki::Configure::Checkers::Tasks::AdministratorEmail
...
sub check {
    ...
    if( defined $TWiki::cfg{Tasks}{AdministratorEmail} &&
        length $TWiki::cfg{Tasks}{AdministratorEmail} ) {

        return '' if( $TWiki::cfg{Tasks}{AdministratorEmail} =~ /\@/ );
        return $this->WARN( "Doesn't look like an e-mail address" );
    }

    return $this->NOTE( "Defaulted to " . ($TWiki::cfg{WebMasterEmail} ||
                                            'no email' ) );
}
