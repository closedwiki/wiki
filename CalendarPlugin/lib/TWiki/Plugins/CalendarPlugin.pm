...
$VERSION    = '1.006';  #ad# support Monthly items
...
sub handleCalendar
{
    ...
    use Date::Calc qw( Date_to_Days Days_in_Month Day_of_Week Nth_Weekday_of_Month_Year);
	...
	$text = &TWiki::Store::readWebTopic($options{web}, $options{topic});
    ...
    @days = fetchDays("$monthly_rx", \$text);
    foreach $d (@days) {
    	($nn, $dd, $descr) = split( /\|/, $d);
