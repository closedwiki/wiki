#!perl
# Install script for BenchmarkContrib
#
...
check the dependencies listed in DEPENDENCIES and assist the user in
installing any that are missing. The script also automatically maintains
the revision histories of any files that are being installed by the
...
my $twiki;
my $NL = "\n";
my %manifest = ( 'BenchmarkContrib_installer.pl'=>0755,'bin/profile'=>0764,'data/Benchmarks/WebAtom.txt'=>0764,'data/Benchmarks/WebChanges.txt'=>0764,'data/Benchmarks/WebHome.txt'=>0764,'data/Benchmarks/WebIndex.txt'=>0764,'data/Benchmarks/WebLeftBar.txt'=>0764,'data/Benchmarks/WebNotify.txt'=>0764,'data/Benchmarks/WebPreferences.txt'=>0764,'data/Benchmarks/WebRss.txt'=>0764,'data/Benchmarks/WebSearch.txt'=>0764,'data/Benchmarks/WebSearchAdvanced.txt'=>0764,'data/Benchmarks/WebStatistics.txt'=>0764,'data/Benchmarks/WebTopicCreator.txt'=>0764,'data/Benchmarks/WebTopicList.txt'=>0764,'data/TWiki/BenchmarkContrib.txt'=>0764,'lib/TWiki/Contrib/BenchmarkContrib.pm'=>0764,'lib/TWiki/Contrib/BenchmarkContrib/DProf.pm'=>0764, );
my @deps = (  );
my $dakar;
my %available;
my $lwp;
...
DONE
    ...
    print <<DONE;
*** BenchmarkContrib depends on $dep->{type} package $dep->{name} $dep->{version}
...
---++ getTWikiCfg($key)
Get the value of an existing configuration key. The key should be in the form
{This}{Key} (e.g. getTWikiCfg("{Plugins}{PluginOrder}");

=cut

sub getTWikiCfg {
    my $key = shift;
    return undef unless ( -e "lib/LocalSite.cfg" );
    open(F, "<lib/LocalSite.cfg") ||
      die "Failed to open lib/LocalSite.cfg for read";
    local $/ = undef;
    my $txt = <F>;
    my $val;
    if ($txt =~ /^.*\$TWiki::cfg$key\s*=\s*(.*?)/) {
        eval '$val='.$1;
    }
    close(F);
    return $val;
...
---++ setTWikiCfg($key, $type, $comment, $value, $nooverwrite)
    * =$key= - key to set, of the form {This}{Key}
    * =$type= - type of value, using =configure= syntax e.g. STRING 30
    * =$comment= - comment to use (no #'s)
    * =$value= - value to set the key to
    * =$nooverwrite= - by default, =setTWikiCfg= will write the key even if it already exists, wiping out any existing type/comment/value setting in =LocalSite.cfg=. If =$nooverwrite= is set, the old type/value will be kept (and the new one ignored).
Returns 1 if the key was set, 0 if it was skipped.

=cut

sub setTWikiCfg {
    my ($key, $type, $comment, $value, $nooverwrite) = @_;
    ...
    if( -e "lib/LocalSite.cfg" ) {
        ...
        $txt =~ s/\s+1;\s*$//s;
        if ($nooverwrite) {
            return 0 if $txt =~ /\$TWiki::cfg$key\s*=/;
        } else {
            # kill the old settings (and previous comment) if any are there
            $txt =~ s/(# \*\*.*?\n(#.*?\n)*)?\$TWiki::cfg$key\s*=.*;\r?\n//s;
        }
    ...
    $txt =~ s/\s+$//s;
    $txt .= "\n".Data::Dumper->Dump([ $value ], [ '$TWiki::cfg'.$key ]);
    $txt .= "\n1;\n";
...
# Try and find an archive for the named module. Look in (1) the current directory
# (2) on the $TWIKI_PACKAGES path and finally (3) download from $PACKAGES_URL
sub getArchive {
    my( $module ) = @_;
    my $f;

    # Look in (1) the current directory (2) $TWIKI_PACKAGES or (3)
    # twiki.org for the module.
    foreach my $dir ('.', split(':', $ENV{TWIKI_PACKAGES}||'')) {
...
sub unzip {
    ...
    unless ( $@ ) {
        ...
        print STDERR 'Archive::Zip is not installed; trying unzip on the command line'.$NL;
        print `unzip $archive`;
        if ( $! ) {
...
sub untar {
    ...
    unless ( $@ ) {
        ...
        print STDERR 'Archive::Tar is not installed; trying tar on the command-line'.$NL;
...
DONE
            ...
            File::Copy::copy($origfile, $tmpfilename) or
              die "$origfile could no be copied to tmp dir ($tmpfilename).";
...
sub uninstall {
    ...
    unless ( $#dead > 1 ) {
        print STDERR 'No part of BenchmarkContrib is installed';
        return 0;
    }
    print 'To uninstall BenchmarkContrib, the following files will be deleted:'.$NL;
    print join( ', ', @dead );
    return 1 if $inactive;
    my $reply = ask('Are you SURE you want to uninstall BenchmarkContrib?');
    if( $reply ) {
        # >>>> PREUNINSTALL
    ...
    print '### BenchmarkContrib uninstalled ###'.$NL;
    return 1;
}
...
sub usage {
    print STDERR <<'DONE';
Usage: BenchmarkContrib_installer -an install
       BenchmarkContrib_installer -an uninstall
       BenchmarkContrib_installer -an upgrade
...
BenchmarkContrib even if they have been locally modified.
...
sub install {
    ...
    if ($path !~ /^TWiki::/) {
        ...
        $path = 'TWiki::'.$type.'::BenchmarkContrib';
    }
    ...
    unless ($@) {
        ...
        if( $version ) {
            unless( ask("BenchmarkContrib version $version is already installed. Are you sure you want to re-install this module?")) {
                return 0;
            }
            print <<DONE;
I am going to uninstall the old BenchmarkContrib before upgrading, to
...
DONE
    ...
    my $archive = getArchive('BenchmarkContrib');
    ...
    print $NL.'### BenchmarkContrib installed';
...
print $NL,'### BenchmarkContrib Installer ###',$NL,$NL;
my $n = 0;
my $action = 'install';
while ( $n < scalar( @ARGV ) ) {
    if( $ARGV[$n] eq '-a' ) {
