# Install script for YetAnotherXpTrackerPlugin
#
...
my $twiki;
my $NL = "\n";
my %manifest = ( 'data/TWiki/XpTrackerPlugin.txt'=>0660,'lib/TWiki/Plugins/XpTrackerPlugin.pm'=>0440,'bin/timesheet'=>0550,'templates/oopstimesheet.tmpl'=>0440,'templates/oopstimesheetreport.tmpl'=>0440,'data/_default_XpTracker/PatternSkinTrackingviewTemplate.txt'=>0440,'data/_default_XpTracker/PatternSkinTrackingeditTemplate.txt'=>0440,'data/_default_XpTracker/PatternSkinTrackinggenericviewTemplate.txt'=>0440,'data/_default_XpTracker/PatternSkinTrackinggenericeditTemplate.txt'=>0440,'lib/TWiki/Plugins/XpTrackerPlugin/build.pl'=>0550,'lib/TWiki/Plugins/XpTrackerPlugin/Business.pm'=>0440,'data/_default_XpTracker/IterationTemplate.txt'=>0660,'data/_default_XpTracker/IterationTemplateOptions.txt'=>0660,'data/_default_XpTracker/ProjectTemplate.txt'=>0660,'data/_default_XpTracker/ProjectTemplateOptions.txt'=>0660,'data/_default_XpTracker/StoryTemplate.txt'=>0660,'data/_default_XpTracker/StoryTemplateOptions.txt'=>0660,'data/_default_XpTracker/TeamTemplate.txt'=>0660,'data/_default_XpTracker/TeamTemplateOptions.txt'=>0660,'data/_default_XpTracker/TeamMembers.txt'=>0660,'data/_default_XpTracker/IterationForm.txt'=>0660,'data/_default_XpTracker/StoryForm.txt'=>0660,'data/_default_XpTracker/ProjectForm.txt'=>0660,'data/_default_XpTracker/ProjectTopics.txt'=>0660,'data/_default_XpTracker/TaskForm.txt'=>0660,'data/_default_XpTracker/ATeamMember.txt'=>0660,'data/_default_XpTracker/WebPreferences.txt'=>0660,'data/_default_XpTracker/AllIterations.txt'=>0660,'data/_default_XpTracker/GettingStarted.txt'=>0660, );
my @deps = ( { name=>'TWiki::Plugins::EditHiddenTablePlugin', type=>'perl',version=>'>=0',description=>'Required. Download and install from [[TWiki:Plugins/EditHiddenTablePlugin][plugin library]].', trigger=>1 }
,{ name=>'TWiki::Plugins::ControlsPlugin', type=>'perl',version=>'>=0',description=>'Required. Download and install from [[TWiki:Plugins/ControlsPlugin][plugin library]].', trigger=>1 }
,{ name=>'HTTP::Date', type=>'cpan',version=>'>=1',description=>'Required. Available from [[http://cpan.uwinnipeg.ca/dist/Time-modules][CPAN]].', trigger=>1 }
,{ name=>'Time::CTime', type=>'cpan',version=>'>=1',description=>'Required. Available from [[http://cpan.uwinnipeg.ca/dist/Time-modules][CPAN]].', trigger=>1 } );
my $dakar;
my %available;
my $lwp;
...
DONE
    ...
    unless ( $ok ) {
        print <<DONE;
*** YetAnotherXpTrackerPlugin depends on $dep->{type} package $dep->{name} $dep->{version}
...
sub uninstall {
    ...
    unless ( $#dead > 1 ) {
        print STDERR 'No part of YetAnotherXpTrackerPlugin is installed';
        return 0;
    }
    print 'To uninstall YetAnotherXpTrackerPlugin, the following files will be deleted:'.$NL;
    print join( ', ', @dead );
    my $reply = ask('Are you SURE you want to uninstall YetAnotherXpTrackerPlugin?');
    if( $reply ) {
        # >>>> PREUNINSTALL
    ...
    print '### YetAnotherXpTrackerPlugin uninstalled ###'.$NL;
    return 1;
}
...
DONE
    ...
    print $NL.'### YetAnotherXpTrackerPlugin installed';
...
sub usage {
    print STDERR <<'DONE';
Usage:YetAnotherXpTrackerPlugin_installer [-a] install
      YetAnotherXpTrackerPlugin_installer [-a] uninstall
      YetAnotherXpTrackerPlugin_installer [-a] upgrade
...
YetAnotherXpTrackerPlugin even if they have been locally modified.
...
print $NL,'### YetAnotherXpTrackerPlugin Installer ###',$NL,$NL;
my $n = 0;
my $action = 'install';
while ( $n < scalar( @ARGV ) ) {
    if( $ARGV[$n] eq '-a' ) {
...
if( $action eq 'upgrade' ) {
    ...
    print <<DONE;
I would like to uninstall YetAnotherXpTrackerPlugin before upgrading, to
...
DONE
    ...
    installPackage( "$PACKAGES_URL/YetAnotherXpTrackerPlugin/", 'YetAnotherXpTrackerPlugin' );
}
