...
sub _renderFormField {
	...
	if ( lc($$def{type}) eq 'label' ) {
		...
		$td = '<select name="'._encode($$def{name}).'" size="'._encode($$def{size}).'">'.$td.'</select>' if $options{mode} ne 'view';
	} elsif  ( lc($$def{type}) eq 'select+multi' ) { 
		$td = _renderOptions($def);
		$td = '<select multiple="multiple" name="'._encode($$def{name}).'" size="'._encode($$def{size}).'">'.$td.'</select>' if $options{mode} ne 'view';
