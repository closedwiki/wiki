...
{ package TopicCryptPluginBuild;

  @TopicCryptPluginBuild::ISA = ( "TWiki::Contrib::Build" );

  sub new {
    my $class = shift;
    return bless( $class->SUPER::new( "TopicCryptPlugin", "Build" ), $class );
  }
...
$build = new TopicCryptPluginBuild();
