...
---+ package WeblistPlugin

=cut

package TWiki::Plugins::WeblistPlugin;
...
sub initPlugin {
    ...
    TWiki::Func::registerTagHandler( 'WEBLIST', \&_WEBLIST );
...
# The function used to handle the %WEBLIST{...}% variable
...
sub _WEBLIST {
    ...
    foreach my $aweb ( @webslist ) {
        if( $aweb eq 'public' ) {
            push( @list, $session->{store}->getListOfWebs( 'user,public,allowed' ) );
        } elsif( $aweb eq 'webtemplate' ) {
            push( @list, $session->{store}->getListOfWebs( 'template,allowed' ));
        } else{
            push( @list, $aweb ) if( $session->{store}->webExists( $aweb ) );
        }
    }
