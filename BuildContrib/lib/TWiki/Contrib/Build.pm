...
The standard module directory structure mirrors the TWiki installation directory structure, so each file in the development directory structure is in the place it will be in in the actual installation. From the root, these are the key areas:
...
---+++ Environment Variables
   ...
   * =$TWIKI_HOME= should point to the root of your test TWiki installation. =$TWIKI_HOME= is where the =install= target will run test installs, and may also be needed for some tests.

---+++ Manifest
The main driving file for the build is the MANIFEST file. This contains a list of all the files that are wanted in the zip file. It is expected to be held in your _module directory_. The MANIFEST file consists of a list of file paths, each relative to the root of the installation. Wildcards may NOT be used. Each file has an optional octal permissions mask and a description; for example,
<verbatim>
...
---+++ Dependencies
The DEPENDENCIES file specifies module dependencies, for example dependencies on other TWiki modules and on CPAN modules etc. The DEPENDENCIES file contains a list of lines, each of which is a single dependency:
<verbatim>
...
sub target_release {
    ...
    $this->sys_action('mv '.$tmpdir.'/'.$project.'.tgz '.$basedir.'/'.
                      $project.'.tgz');
    print 'Release ZIP is '.$basedir.'/'.$project.'.zip',$NL;
    print 'Release TGZ is '.$basedir.'/'.$project.'.tgz',$NL;
