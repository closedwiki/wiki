...
sub mdrepo {
	...
	TWiki::UI::checkAccess($session, undef, undef, 'CHANGE', $session->{user});
	# see if the user is in ALLOWROOTCHANGE
	if ( $req->param("_add") ) {
	    $cmdName = "add";
	}
	...
	if ( $cmdSpec && $table ) {
		...
		$output .= "table $table is not allowed to be modified via CGI.\n";
	    }
	}
	else {
	    if ( !$cmdSpec ) {
