...
sub upload {
    ...
    my @filePaths   = $query->param('filepath');
    my @fileNamesTmp = $query->param('filepath');

    my @fileNames = ();
    foreach (@fileNamesTmp) {
        push @fileNames, $_
          unless $_ eq '';    #If HTML form has more input type=file fields,
                              #But files are not uploaded for all fields, then
                              #CGI assigns them ''
    }
    ...
    my @streams = $query->upload('filepath');
