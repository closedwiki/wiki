...
sub test_simple {
  ...
  my $old = $this->getOld("attach", $web, $topic,
                          "filename=/tmp/robot.gif");
  $this->oldPubExists($web, $topic, "robot.gif");
  $this->createTempForUpload();
  my $new = $this->getNew("attach", $web, $topic,
                          "filename=/tmp/robot.gif");
  $this->newPubExists($web, $topic, "robot.gif");
  $this->diff($old, $new);
}
