...
# should be run in an empty directory. The downloads are written to two
# subdirectories created where the script is run, repository and download.
#
...
# -used will automatically assume the last zip downloaded was the right one, and
# will suppress the prompt for which file to download if there is a choice.
#
use strict;
use Time::ParseDate;
use CGI qw( -any );
...
sub getPackageList {
    ...
    if ($text) {
        foreach my $line ( split( /\n/, $text ) ) {
            if ( $line =~ /<a .*?href=\"$View\/Plugins\/(\w+(Plugin|Skin|AddOn|Contrib))/o ) {
                my $p = $1;
                push( @$packages, $p ) unless ( grep( /^$p$/, @$packages ) );
            }
        }
    }
    unlink( "Plugins.$packs" );
}
...
sub updateCVS {
    ...
    if ( -d "repository/twikiplugins/$module" ) {
        ...
        print STDERR "--- Checking out CVS $module\n";
