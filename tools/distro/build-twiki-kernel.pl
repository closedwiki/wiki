...
my $result = GetOptions(
			'localcache=s' => \$Config->{local_cache},
			'tempdir=s' => \$Config->{temp_dir},
			'outputdir=s' => \$Config->{output_dir},
...
$Config->{install_base} = $Config->{temp_dir} . "/twiki";		# the directory the official release is untarred into


################################################################################

if (( ! -e $Config->{temp_dir} ) || ( ! -d $Config->{temp_dir} )) {
    print "Error: $Config->{temp_dir} does not exist, or is not a directory\n";
    exit(2);
}
if (( ! -e $Config->{output_dir} ) || ( ! -d $Config->{output_dir} )) {
    print "Error: $Config->{output_dir} does not exist, or is not a directory\n";
    exit(2);
}

if ( $Config->{verbose} )
{
    print "temporary files will go into $Config->{temp_dir}\n";
    print "output tar file will go into $Config->{output_dir}\n";
}
...
execute( "cd $Config->{temp_dir} ; tar xzf $localTar" ) or die $!;
if ( $Config->{verbose} ) { print scalar File::Find::Rule->file->in( $installBase ), " original files\n" }
...
# deletes files/directories which *are* bundled from svn
...
my $newDistro = "$Config->{output_dir}/$newFile";
execute( "cd $Config->{temp_dir} ; tar czf ${newDistro}.tar.gz twiki" ) or die $!;	# .tar.gz goes *here* because *z* is here
...
B<build-twiki-kernel.pl> will build a TWikiKernel release file suitable for distribution.
...
Notably the TWikiKernel does not include any TWikiExtensions (e.g. Plugins or Skins). TWikiExtensions (Plugins and Skins) are bundled into a particular TWikiRelease (or TWikiDistribution).
