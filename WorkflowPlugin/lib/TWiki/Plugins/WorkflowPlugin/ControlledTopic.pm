...
sub isModifyable {
    ...
    unless (defined $this->{isEditable}) {
        $this->{isEditable} = (
            ...
            # Does TWiki permit editing?
            && TWiki::Func::checkAccessPermission(
                'CHANGE', $TWiki::Plugins::SESSION->{user},
...
sub changeState {
    ...
    if ($notify) {
        ...
        foreach my $who (@persons) {
            if ( $who =~ /^$TWiki::regex{emailAddrRegex}$/ ) {
                ...
                if ( scalar(@list) ) {
                    ...
                    TWiki::Func::writeWarning( __PACKAGE__
        ...
        if ( scalar(@emails) ) {
            ...
            TWiki::Func::setPreferencesValue( 'EMAILTO',
                join( ', ', @emails ) );
            TWiki::Func::setPreferencesValue( 'TARGET_STATE',
                $this->getState() );
