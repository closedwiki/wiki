...
HERE
    $this->{twiki}->{store}->saveAttachment($this->{users_web}, "TopicWithHtmlAttachment", "Simple_example.html",
                                            $this->{twiki}->{user}, {file => "attachement_examples/Simple_example.html"})
}
...
sub test_stringForFile {
    ...
    my $text2 = TWiki::Contrib::SearchEngineKinoSearchAddOn::Stringifier->stringFor('attachement_examples/Simple_example.html');
