...
sub indexer {
    ...
    $invindexer->spec_field( name  => 'topic',
			     boost => 3 );
    ...
    $invindexer->spec_field( name  => 'id_topic',
			     analyzed => 0);
    ...
    $invindexer->spec_field( name => 'version',
			     analyzed => 0 );
...
sub removeTopics
{
	...
	if ( @attachments ) {
		...
		if (($indexextensions{".$extension"})&&(!$skipattachments{"$web.$topic.$name"})) {
		    ...
		    my $term = KinoSearch::Index::Term->new( 'id_topic', $web.$topic.$name );
