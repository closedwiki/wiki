...
@registrableHandlers = (
        ...
        'initializeUserHandler',   # ( $theRemoteUser, $url, $pathInfo )
        'registrationHandler'      # ( $ webName, $wikiName )
    );
...
#&TWiki::writeDebug("registerHandler: $handlerName");
}
...
sub applyHandlers
{
    ...
    foreach $theHandler ( @{$registeredHandlers{$handlerName}} ) {
        ...
        $status = &$theHandler;
#&TWiki::writeDebug("$status = applyHandlers ($handlerName) ");
...
#    
#    return $user;
#&TWiki::writeDebug("Plugin::initializeUser");
