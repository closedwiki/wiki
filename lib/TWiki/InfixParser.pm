...
---++ new($client_class, \@opers, %options) -> parser object

Creates a new infix parser using the operators specified in the \@opers array.
The array contains one entry for each operator. Each entry is a hash,
...
generated by this parser will point to the hashes in \@opers. Field names
starting with =InfixParser_= are reserved for use by the parser.
...
sub new {
    ...
    $this->{standard_op_REs} = join('|', map {
        $_->{InfixParser_RE}
    } values %{$this->{standard_ops}});

    # and repeat for bracket operators
    $this->{bracket_op_REs} = join('|', map {
        $_->{InfixParser_RE}
    } values %{$this->{bracket_ops}});
    ...
    try {
        while ($$input =~ /\S/) {
            if ($$input =~ s/^\s*($this->{standard_op_REs})//) {
                ...
                my $val = $2;
                push( @opands, $this->{client_class}->newLeaf($val, 3));
            } elsif ($$input =~ s/^\s*($this->{numbers})//) {
                print STDERR "Tok: number '$1'\n" if MONITOR_PARSER;
                my $val = $1;
                push( @opands, $this->{client_class}->newLeaf($val, 2));
            } elsif ($$input =~ s/^\s*($this->{words})//) {
                print STDERR "Tok: word '$1'\n" if MONITOR_PARSER;
                my $val = $1;
                push( @opands, $this->{client_class}->newLeaf($val, 1));
