...
# $Rev: 4729 $ so that TWiki can determine the checked-in status of the
# extension.
our $VERSION = '$Rev: 4729 $';
...
sub postRenderingHandler {
	...
	return if !_isTrackingEnabledForScript( $ENV{TWIKI_ACTION} );
...
sub _htmlTag {
	...
	my $key = $TWiki::cfg{Plugins}{GoogleAnalyticsPlugin}{WebPropertyId};
	_debug("sub _htmlTag");
	($key) or _debug("\t no {WebPropertyId} key found");
	if (!$key) {
		# still support GOOGLESITEKEY as long as {WebPropertyId} is not entered
		$key = TWiki::Func::getPreferencesValue( "GOOGLESITEKEY", $web );
...
sub _isTrackingEnabledInSetting {
	...
	$value ||= '';
    my $setting = $TWiki::cfg{Plugins}{$pluginName}{Tracking}{$mode}{$key} || '';
...
sub _debug {
	my ($text) = @_;
	return if !$TWiki::cfg{Plugins}{GoogleAnalyticsPlugin}{Debug};
