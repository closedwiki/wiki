...
$SHORTDESCRIPTION = 'Ping a host and get a response';
...
sub initPlugin {
    ...
    # $TWiki::cfg{Plugins}{PingPlugin}{ExampleSetting} = 1;
    # Always provide a default in case the setting is not defined in
    # LocalSite.cfg. See TWiki.TWikiPlugins for help in adding your plugin
    # configuration to the =configure= interface.
    my $setting = $TWiki::cfg{Plugins}{PingPlugin}{ExampleSetting} || 0;
    $debug = $TWiki::cfg{Plugins}{PingPlugin}{Debug} || 0;
...
sub commonTagsHandler {
    ...
    $_[0] =~ s/%PING{\s*host="(.+?)"\s+wait="(.+?)"\s*}%/&handlePing($1, $2)/ges;
...
sub handlePing
{
    ...
    unless ($wait =~ /^-?[0-9]+$/)
    {
        return qq(%RED% PING{host="$host" wait="$wait"} : Not a number %ENDCOLOR%);
    }

    TWiki::Func::writeDebug( "- ${pluginName}::handlePing(\n host: $host\n wait: $wait\n )" ) if $debug;
    my $ret = "";
    ...
    if ( $p->ping($host, $wait) )
    {
            $ret = "%GREEN%$host%ENDCOLOR%";
    }
    else
    {
            $ret = "%RED%$host%ENDCOLOR%";
    }

    $p->close();

    TWiki::Func::writeDebug( "- ${pluginName}::handlePing() returns:\n$ret" ) if $debug;
    return $ret;
}
