...
my $funcUsageReport = "";
foreach my $key (sort {$data{funcsyms}{$a} <=> $data{funcsyms}{$b}} keys %{$data{funcsyms}}) {
...
foreach my $token (@badtoks) {
  ...
  $illegalCallsReport .= TR(TD("<nop>$token"),TD($data{howbad}{$token}),
		    TD(join(" ", @badmods)));
}
...
foreach $module (sort keys %{$data{suspect}}) {
...
foreach my $find (sort keys(%{$data{directives}})) {
  $directivesReport .=
    TR(TD($find),TD(join(", ",sort(keys %{$data{directives}{$find}}))));
}
...
sub compareCVS2Download {
  ...
  if (-d "repository/twikiplugins/$module" &&
      -d "download/$module") {
    my $diffs = `diff -rq download/$module repository/twikiplugins/$module | grep -v ": CVS"| grep -v ": $zip"`;
    if ($?) {
      print STDERR $diffs;
      return;
    }
...
sub getAttachments {
  ...
  foreach my $line (split(/\n/,$text)) {
    if ($line =~ m/^<IMG.*?>\s*<A HREF=\"([\w_\.]+)\">.*?<\/A>\s*([\w\-]+\s*[\d:]+)\s+\w*/i) {
      my $file = $1;
      my $time = $2;
...
sub updateCVS {
  ...
  if (-d "repository/twikiplugins/$module") {
    ...
    `cd repository/twikiplugins/$module && cvs -d:pserver:anonymous\@cvs.TWiki.sourceforge.net:/cvsroot/twikiplugins update $quiet`;
    if ($?) {
      print STDERR "CVS update on $module failed: $?\n";
    }
  } else {
    print "--- Checking out CVS $module\n";
    `cd repository && cvs -d:pserver:anonymous\@cvs.TWiki.sourceforge.net:/cvsroot/twikiplugins checkout twikiplugins/$module $quiet`;
  }
}
...
sub updateDownload {
  my $module = shift;
  my $atts = getAttachments("Plugins", $module,
			    qr/\.(zip|tgz|tar\.gz|pm)$/);
  ...
  while (scalar(%$atts)) {
    ...
    `wget -q $url`;
    if (!$? && -f $chosen) {
      `chmod -R 777 download/$module && rm -rf download/$module` if (-d "download/$module");
